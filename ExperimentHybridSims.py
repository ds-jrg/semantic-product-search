...
# Calculating the similaritiess, through both similarities of GNN and Textbased approach
# Prompt:
...
def train(train_dataloader):
    ...
    for querys, qids, product_ids in train_dataloader:
        ...
        l1 = F.cosine_similarity(anchor_projs, old_emb[node_ids], dim=1)
        l2 = F.cosine_similarity(anchor_projs, product_emb, dim=1)
        final_sims = l2 * (1 - alpha) + l1 * alpha
        loss = F.mse_loss(final_sims, labels)
...
def test(dataloader):
    ...
    with torch.no_grad():
        ...
        for anchors, qids, _ in dataloader:
            ...
            o_sims = sentence_transformer_model.similarity(anchors, old_emb)
            for i in range(len(anchors)):
                sim_scores = sims[i].cpu().numpy()
                o_sim_scores = o_sims[i].cpu().numpy()
                final_sims = sim_scores * (1 - alpha) + o_sim_scores * alpha
                qid = qids[i].item()
                # Represents the node-indexes with decreasing similarity
                sim_indx = np.argsort(final_sims)[::-1].tolist()
...
def perfrom_Training_and_Evaluation(train_dataloader, eval_dataloader, test_dataloader, model):
    ...
    patience = 14  # how many epochs to wait
